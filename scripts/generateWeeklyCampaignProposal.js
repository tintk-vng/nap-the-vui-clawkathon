...
function buildArticleContent({ publisher, targetGameNames, discountText, discountSource, bestResearchSignal, briefTitle }) {
  ...
  return [
    `NapTheVui đang chuẩn bị ưu đãi nạp ${publisher.displayName} dành cho người dùng muốn nạp nhanh các tựa game quen thuộc.`,
    '',
    `Áp dụng cho: ${gameText}.`,
    discountLine,
    sourceLine,
    '',
    'Cách sử dụng:',
    `1. Tìm ${publisher.displayName} hoặc tên game trong ô tìm kiếm NapTheVui.`,
    ...
    '',
    `Lưu ý: Bài viết này được tạo từ ${briefTitle}. Các chi tiết khuyến mãi cần được xác nhận trong preview và validation trước khi xuất hiện trên website chính thức.`
  ].join('\n')
}
...
function buildProposal({
  ...
  return {
    ...
    bannerTitle,
    bannerSubtitle,
    bannerImageUrl: bestManualSignal.bannerImageUrl || publisher.logoUrl,
    mobileBannerImageUrl: bestManualSignal.mobileBannerImageUrl || bestManualSignal.bannerImageUrl || publisher.logoUrl,
    ctaText,
