...
export default function BannerSlot({ campaign }: BannerSlotProps) {
  ...
  const content = (
    ...
    >
      ...
      <div className="relative z-10 flex h-full items-center justify-between gap-4">
        ...
        <div className="hidden h-20 w-28 shrink-0 items-center justify-center rounded-lg bg-white-500/95 p-4 shadow-soft sm:flex md:h-24 md:w-32">
  ...
  return (
    <section className="mb-7 overflow-hidden rounded-lg bg-white-500 shadow-soft md:mb-8">
      {campaign.articleId ? (
        <Link href={`/mua-the-game/tin-tuc/${campaign.articleId}`} className="block">
          {content}
        </Link>
      ) : (
        content
